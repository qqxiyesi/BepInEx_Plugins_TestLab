...
namespace Specter
{
	...
	public class Specter : BaseUnityPlugin
	{
		...
		private void Awake()
		{
			...
			ConfigRatio = Config.Bind("General", "UI Ratio", 1f, new ConfigDescription("", new AcceptableValueRange<float>(0.1f, 10f)));

			StudioAPI.StudioLoadedChanged += (sender, e) => gameObject.GetOrAddComponent<StudioCommonGuide>();

			var iconTex = LoadTexture(ResourceUtils.GetEmbeddedResource("toolbar_icon.png")); // from StudioExtraMoveAxis
